#!/usr/bin/env python
# parse pcap_test fields from pcap_test buffers into a dictionary
#
# example:
# >>> import pcap_test as r, pcap_test
# >>> pc = pcap_test.pcap_test(name='foo.pcap_test')
# >>> tstamp, pkt = pc[0]
# >>> off, pcap_test = r.radiotap_parse(pkt)
# >>> off, mac = r.ieee80211_parse(pkt, off)
import struct

from vmacinfer.common.pcap import *
...
def _parse_ampdu(packet, offset):
    """see http://www.pcap.org/defined-fields/A-MPDU%20status
...
def _parse_vht(packet, offset):
    """ see http://www.pcap.org/defined-fields/VHT
...
def _parse_radiotap_field(namespace, field_id, packet, offset):
    if namespace and namespace != 'pcap_test':
        # skipped already
        return offset, None
...
def radiotap_parse(packet, valuelist=False):
    """
    Parse out a the pcap_test header from a packet.  Return a tuple
